...
MAX_GEOMETRY_VALUE = 100.0
MAX_CONFIDENCE = 1000.0


def bounded_expp1(logits: torch.Tensor) -> torch.Tensor:
    """Compute ``1 + exp(logits)`` without allowing confidence overflow."""

    log_confidence = F.softplus(logits).clamp(max=math.log(MAX_CONFIDENCE))
    return torch.exp(log_confidence).clamp(max=MAX_CONFIDENCE)
...
class DenseHead(nn.Module):
    ...
    def _forward_impl(
        ...
        depth = torch.exp(depth_logits.clamp(max=math.log(MAX_GEOMETRY_VALUE))).clamp(
