...
try:
    # FlashAttention-3 exposes a top-level hopper interface.
    from flash_attn_interface import flash_attn_func
except ImportError:
    try:
        # FlashAttention-2 exports the function from the package root.
        from flash_attn import flash_attn_func
    except ImportError:
        # FlashAttention-4 keeps the CuTe implementation under flash_attn.cute.
...
class WanRotaryPosEmbed(nn.Module):
    def __init__(
        ...
        self.register_buffer("f_freqs_base", f_freqs_base)
        self.register_buffer("h_freqs_base", h_freqs_base)
        self.register_buffer("w_freqs_base", w_freqs_base)
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward_train(self, input_dict):
        ...
        # latent_dict['grid_id'] [1, 4, 2464] -> [1, 4, 2464]
        latent_grid_id = latent_dict['grid_id'].permute(1, 0, 2).flatten(1)[None]
        # action_dict['grid_id'] [1, 4, 308] -> [1, 4, 308]
        ...
        latent_temb, latent_timestep_proj =self._time_embed(latent_time_steps, 
                        ...
                        action_mode=False)
        # action_temb -> [1, 616, 3072]     action_timestep_proj -> [1, 616, 6, 3072]
